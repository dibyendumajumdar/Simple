...
public class DominatorTree {
    ...
    private void populateTree() {
        for (BasicBlock block : _blocks) {
            BasicBlock idom = block._idom;
            if (idom == block) // root
                continue;
            // add edge from idom to n
            idom._dominated.add(block);
        }
    }
    ...
    private void setDepth_(BasicBlock block) {
        ...
        if (idom != block) {
            ...
            block._domDepth = idom._domDepth + 1;
        } else {
            assert idom._domDepth == 1;
            assert idom._domDepth == block._domDepth;
        }
        for (BasicBlock child : block._dominated)
            setDepth_(child);
    }
